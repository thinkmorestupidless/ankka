...
object Settings:
  ...
  /**
   * `-Dankka.config`, else `$ANKKA_CONFIG`, else `~/.ankka/config.json`.
   *
  ...
  def path: Path =
    sys.props
      .get("ankka.config")
      .orElse(sys.env.get("ANKKA_CONFIG"))
      .map(Paths.get(_))
      .getOrElse(Paths.get(sys.props("user.home"), ".ankka", "config.json"))
   ...
   * anything. `ankka config set` overwrites it.
   */
  def load(): Settings =
  ...
  def resolve(
    ...
    val saved = load()
    Settings(
      url = urlFlag.orElse(sys.env.get("ANKKA_URL")).getOrElse(saved.url),
      token = tokenFlag.orElse(sys.env.get("ANKKA_TOKEN")).orElse(saved.token),
      project = projectFlag.orElse(sys.env.get("ANKKA_PROJECT")).orElse(saved.project),
      ca = sys.env.get("ANKKA_CA").orElse(saved.ca)
    )
