 ...
 *
 * Every test points `ankka.config` at a scratch file, so nothing here can read or write the
 * developer's own `~/.ankka/config.json`.
 */
class SettingsSuite extends munit.FunSuite:

  private val scratch = FunFixture[Path](
    setup = _ =>
      val file = Files.createTempFile("ankka-cli", ".json")
      Files.delete(file)
      sys.props("ankka.config") = file.toString
      file
    ,
    teardown = file =>
  ...
  scratch.test("settings round-trip through the file") { _ =>
    val _ = Settings.save(
      Settings("http://cp:8080", Some("secret"), Some("checkout"), Some("~/.ankka/local-ca.crt"))
    )
    val loaded = Settings.load()
    ...
    assertEquals(loaded.ca, Some("~/.ankka/local-ca.crt"))
  }
  ...
  scratch.test("save creates the parent directory") { file =>
    val nested = file.resolveSibling("ankka-cli-nested").resolve("deeper").resolve("config.json")
    sys.props("ankka.config") = nested.toString
    try
  ...
  scratch.test("the saved file supplies whatever the flags leave out") { _ =>
    // The environment sits between the flags and the file, so a developer with ANKKA_URL
    // exported would legitimately see a different answer here.
    assume(
      Seq("ANKKA_URL", "ANKKA_TOKEN", "ANKKA_PROJECT").forall(!sys.env.contains(_)),
      "ANKKA_* is set in this environment"
    )
    val _ = Settings.save(Settings("http://saved", Some("saved-token"), Some("saved-project")))
