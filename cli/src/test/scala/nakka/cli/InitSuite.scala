...
/** What `nakka init` decides before it starts a process. */
class InitSuite extends munit.FunSuite:

  test("the sbt invocation carries the template, the name and this CLI's version") {
    val cmd = Init.command(Init.Request("orders", "file:///t/nakka.g8"), version = "0.2.0")
    assertEquals(cmd.take(3), Vector("sbt", "--allow-empty", "-batch"))
    assertEquals(cmd(3), "new file:///t/nakka.g8 --name=orders --nakka_version=0.2.0")
  }
  ...
  test("the default template is the published one") {
    assert(
      Init.newCommand(Init.Request("orders"), "1.0.0").contains("thinkmorestupidless/nakka.g8")
    )
  }
