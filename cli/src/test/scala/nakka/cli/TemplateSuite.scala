...
/**
 * The feature's proof (006): a build outside this one resolves nakka and its own tests pass.
 *
 * `build.sbt` publishes the six artifacts locally before this suite runs (`cli / Test / test`
 * depends on `publishLocal`), then this expands the template through `nakka init` — the real
...
class TemplateSuite extends munit.FunSuite:
  ...
  override def munitIgnore: Boolean =
    sys.props.get("nakka.template.tests").contains("off") || !Init.sbtOnPath()

  private val Name    = "probe"
  private val Version = nakka.core.BuildInfo.version
  ...
  override def beforeAll(): Unit =
    if !munitIgnore then
      workspace = Files.createTempDirectory("nakka-template")
      val request =
        Init.Request(Name, s"file://${repoRoot.resolve("nakka.g8")}", directory = workspace)
