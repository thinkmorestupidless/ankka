...
class ServiceProjectionSuite extends munit.FunSuite:
  ...
  test("a literal env var and a secret reference both survive the crossing") {
    val withEnv = descriptor(env =
      Vector(
        ...
        EnvVar("ANKKA_DB_PASSWORD", secretKeyRef = Some(SecretKeyRef("cart-db", "password")))
      )
    )
  ...
  test("a descriptor with no ANKKA_DB_* env var provisions a database") {
    val Right(spec) = ServiceProjection.project(service(), config): @unchecked
    assertEquals(spec.provisionDatabase, true)
  }

  test(
    "a descriptor declaring its own ANKKA_DB_* env var is the escape hatch — no database provisioned"
  ) {
    val withEnv = descriptor(env = Vector(EnvVar("ANKKA_DB_HOST", value = Some("db.example.com"))))
    val Right(spec) = ServiceProjection.project(service(d = withEnv), config): @unchecked
  ...
  test("the namespace is one per project") {
    assertEquals(config.namespaceFor("checkout"), "ankka-checkout")
  }
