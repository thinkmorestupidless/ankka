...
class ExposureClusterSuite extends munit.FunSuite:
  ...
  override def munitIgnore: Boolean =
    sys.props.get("nakka.cluster.tests").contains("off") || !curlAvailable
  ...
  override def beforeAll(): Unit =
    if !munitIgnore then
      ...
      config = Files.createTempFile("nakka-exposure", ".json")
      Files.delete(config)
  ...
  private def status(project: String, name: String) =
    Option(
      k8s.resources(classOf[NakkaService]).inNamespace(s"$Prefix-$project").withName(name).get()
  ...
  test("3. expose: the CLI prints the URL, and within 60s a cart written by hostname reads back") {
    val (code, out) = nakka("services", "expose", Service, "-p", Project)
    ...
    val (_, got) = nakka("services", "get", Service, "-p", Project)
  ...
  test("5. the same name in another project is another hostname with its own state") {
    assertEquals(nakka("projects", "create", Other, "--name", "Returns", "-O", "acme")._1, 0)
    apply(Other, Service)
    waitReady(Other, Service, 1)
    assertEquals(nakka("services", "expose", Service, "-p", Other)._1, 0)
  ...
  test("8. unexpose removes only the route: 404 at the hostname, the service untouched") {
    ...
    val (code, out) = nakka("services", "unexpose", Service, "-p", Project)
    assertEquals(code, 0, out)
  ...
  test("9. deleting a project takes its route with it — nothing in the cluster names it") {
    assertEquals(nakka("services", "delete", Service, "-p", Other)._1, 0)
    waitFor(60.seconds)(routeOf(Other, Service).isEmpty)
    waitFor(60.seconds)(nakka("services", "list", "-p", Other)._2.contains("no results"))
    assertEquals(nakka("projects", "delete", Other)._1, 0)
    val everywhere = k8s
