...
import nakka.controlplane.deploy.{NakkaServiceClient, NakkaServiceResource}
 ...
 */
final class FakeNakkaServiceClient extends NakkaServiceClient:

  private val resources = TrieMap.empty[(String, String), NakkaServiceResource]
  private val watchers  = TrieMap.empty[Int, NakkaServiceResource => Unit]
  ...
  def put(namespace: String, name: String, spec: NakkaServiceSpec): Unit =
    ...
    if !existing.exists(_.spec == spec) then
      ...
      val updated = NakkaServiceResource(
        namespace,
        name,
        spec,
  ...
  def setStatus(namespace: String, name: String, status: NakkaServiceStatus): Unit =
  ...
  def driftEdit(namespace: String, name: String)(f: NakkaServiceSpec => NakkaServiceSpec): Unit =
    resources.get((namespace, name)).foreach { current =>
  ...
  def current(namespace: String, name: String): Option[NakkaServiceResource] =
    resources.get((namespace, name))

  def all: Vector[NakkaServiceResource] = resources.values.toVector
  ...
  private def notifyWatchers(resource: NakkaServiceResource): Unit =
