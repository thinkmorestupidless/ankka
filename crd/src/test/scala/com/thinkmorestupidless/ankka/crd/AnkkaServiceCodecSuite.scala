 ...
 */
class AnkkaServiceCodecSuite extends munit.FunSuite:

  private val serialization = AnkkaSerialization()

  private val fullSpec = AnkkaServiceSpec(
    projectId = "checkout",
    serviceName = "cart",
    generation = 4L,
    paused = false,
    ...
    env = List(
      ...
      EnvEntry("ANKKA_DB_PASSWORD", secretName = Some("cart-db"), secretKey = Some("password"))
    ),
  ...
  test("a fully populated spec round-trips unchanged") {
    ...
    val decoded = serialization.unmarshal(json, classOf[AnkkaServiceSpec])
  ...
  test("a spec missing fields decodes to their defaults") {
    ...
    val decoded = serialization.unmarshal(sparse, classOf[AnkkaServiceSpec])
  ...
  test("an unknown field is tolerated rather than fatal") {
    ...
    val decoded = serialization.unmarshal(future, classOf[AnkkaServiceSpec])
  ...
  test("the whole resource round-trips with spec and status independent") {
    val resource = AnkkaService("ankka-checkout", "cart", fullSpec)
    resource.setStatus(AnkkaServiceStatus(generation = 4L, lifecycle = "Ready", readyInstances = 1))

    val decoded = serialization.unmarshal(serialization.asJson(resource), classOf[AnkkaService])
  ...
  test("identity comes from the annotations, so it cannot drift from what fabric8 uses") {
    assertEquals(AnkkaServiceDefinition.group, "ankka.thinkmorestupidless.com")
    assertEquals(AnkkaServiceDefinition.version, "v1alpha1")
    assertEquals(AnkkaServiceDefinition.kind, "AnkkaService")
    assertEquals(AnkkaServiceDefinition.plural, "ankkaservices")
    assertEquals(AnkkaServiceDefinition.apiVersion, "ankka.thinkmorestupidless.com/v1alpha1")
    assertEquals(AnkkaServiceDefinition.crdName, "ankkaservices.ankka.thinkmorestupidless.com")
  }

  test("sameReport ignores the clock") {
    val a = AnkkaServiceStatus(lifecycle = "Ready", lastTransitionTime = "2026-09-14T10:00:00Z")
    val b = AnkkaServiceStatus(lifecycle = "Ready", lastTransitionTime = "2026-09-14T11:00:00Z")
  ...
  test(
    ...
    val sparse = """{"projectId":"checkout","serviceName":"cart","generation":1,"image":"img:1"}"""
    assertEquals(serialization.unmarshal(sparse, classOf[AnkkaServiceSpec]).provisionDatabase, true)
  }
  ...
  test("port is absent by default, so a resource from before it existed still serves no HTTP") {
    val sparse = """{"projectId":"checkout","serviceName":"cart","generation":1,"image":"img:1"}"""
    assertEquals(serialization.unmarshal(sparse, classOf[AnkkaServiceSpec]).port, None)
  }
  ...
  test("a status with no database field decodes to None") {
    ...
    assertEquals(
      serialization.unmarshal(withoutDatabase, classOf[AnkkaServiceStatus]).database,
      None
    )
  }
