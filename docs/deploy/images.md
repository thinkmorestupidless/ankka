...
description: Package a Scala or Python ankka service as a container image, tag it, and get it onto a cluster by pushing to a registry or loading it into a local kind node.
kind: guide
...
platform runs. For a project that depends on the SDK from PyPI (`ankka==0.5.0` in its `pyproject.toml`),
