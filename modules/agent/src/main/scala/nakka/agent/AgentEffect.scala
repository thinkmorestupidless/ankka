...
object ResponseShape:
   ...
   *
   * The parse failure is surfaced rather than swallowed: a model that returned prose
   * where JSON was asked for is a prompt problem, and silently returning a default would
   * hide it.
   */
  final class AsJson[R](codec: JsonValueCodec[R], val schemaHint: String)
      extends ResponseShape[R]:
    def decode(text: String): Either[String, R] =
 ...
 *
 * Inert, like every other nakka effect: building it calls no model and reads no memory.
 * That is what lets an agent's decision-making be unit-tested against a scripted
 * provider with nothing running.
 *
 * A plain class rather than a `case class`, and its fields are named differently from
 * its methods on purpose. A field called `tools` would shadow the `tools(...)` builder —
 * `effect.tools(x)` would silently resolve to `Vector.apply(index)` and fail with a type
 * error about `Int`.
 */
final class AgentEffect[R] private[agent] (
   ...
   *
   * Kept separate from `userMessage` so memory records what the user actually said
   * rather than the whole assembled prompt.
   */
   ...
   * only decodes, so a mismatch surfaces as a decode error naming the type.
   */
 ...
 *
 * Returning `Left` rejects the interaction. Input guardrails run before any model is
 * called, so a rejection there costs nothing; output guardrails run before memory is
 * written, so a rejected reply leaves no trace in the conversation.
 *
 * Guardrails run in declaration order and stop at the first rejection — put the cheap
 * checks first.
 */
trait Guardrail:
  def name: String
