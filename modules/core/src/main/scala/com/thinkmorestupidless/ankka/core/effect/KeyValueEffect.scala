...
sealed trait KeyValueEffect[S, +R]:
  private[ankka] def newState: Option[S]
...
sealed trait KeyValueReadOnlyEffect[S, +R] extends KeyValueEffect[S, R]:
  private[ankka] final def newState: Option[S]          = None
  private[ankka] final def retention: Option[Retention] = None

object KeyValueEffect:

  private[ankka] final case class Updating[S, R](
      newState: Option[S],
      retention: Option[Retention],
      outcome: Outcome[S, R]
  ) extends KeyValueEffect[S, R]

  private[ankka] final case class ReadOnly[S, R](
      outcome: Outcome[S, R]
  ...
  private[ankka] def materialise[S, R](
...
final class KeyValueEffects[S] private[ankka] ():
...
final class UpdateBuilder[S] private[ankka] (
