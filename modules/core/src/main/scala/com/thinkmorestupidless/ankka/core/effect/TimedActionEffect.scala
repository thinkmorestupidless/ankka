 ...
 *
 * Timers are at-least-once and are only removed once they succeed, so `Fail` means "reschedule me".
 * A timed action for work that has become obsolete must therefore return `done()`, not fail —
 * otherwise it retries forever. This is the sharpest edge in the timer API and the reason the two
 * cases are named this bluntly.
 */
...
final class TimedActionEffects private[ankka] ():
