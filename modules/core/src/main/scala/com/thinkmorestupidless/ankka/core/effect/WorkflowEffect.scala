...
sealed trait WorkflowEffect[S, +R]:
  private[ankka] def stateChange: Option[S]
  private[ankka] def transition: Option[StepRef]
  private[ankka] def deleting: Boolean
  private[ankka] def outcome: Outcome[S, R]

/** A command effect that provably changes nothing. */
sealed trait WorkflowReadOnlyEffect[S, +R] extends WorkflowEffect[S, R]:
  private[ankka] final def stateChange: Option[S]      = None
  private[ankka] final def transition: Option[StepRef] = None
  private[ankka] final def deleting: Boolean           = false

object WorkflowEffect:

  private[ankka] final case class Changing[S, R](
  ...
  private[ankka] final case class ReadOnly[S, R](outcome: Outcome[S, R])
      extends WorkflowReadOnlyEffect[S, R]

/** What a step does when it finishes. */
sealed trait WorkflowStepEffect[S]:
  private[ankka] def stateChange: Option[S]
  private[ankka] def next: StepOutcome

object WorkflowStepEffect:
  private[ankka] final case class Impl[S](stateChange: Option[S], next: StepOutcome)
      extends WorkflowStepEffect[S]

/** The `effects` surface in a workflow's command handlers. */
final class WorkflowEffects[S] private[ankka] ():
...
final class WorkflowCommandBuilder[S] private[ankka] (
...
final class WorkflowStepEffects[S] private[ankka] ():
...
final class WorkflowStepBuilder[S] private[ankka] (private val stateChange: Option[S]):
  import WorkflowStepEffect.*
