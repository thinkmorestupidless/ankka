 ...
 * the field, splitting one token into two events. Both are silent corruptions that only show up on
 * text a model happened to generate.
 *
 * Encoding each event as a JSON string makes the framing unambiguous and the payload byte-exact.
 * Clients parse the `data:` value as JSON.
 */
