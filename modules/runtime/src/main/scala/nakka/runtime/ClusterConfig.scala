 ...
 *
 * {{{
 *   system properties  >  the service's application.conf  >  nakka-cluster-<mode>.conf  >  reference.conf
 * }}}
 *
...
object ClusterConfig:
  ...
  val ModeVariable: String = "NAKKA_CLUSTER_MODE"

  /** One overlay file per entry, `nakka-cluster-<mode>.conf`. */
   ...
   *   default, an explicit map in tests. Only the platform's variables (`NAKKA_*`, `POD_IP`) are
   ...
   *
   * `Nakka.start` passes every config through here. One the loader produced is returned as it is.
  ...
  private def overlay(env: Map[String, String]): Config =
    ...
    ConfigFactory.parseResources(s"nakka-cluster-$mode.conf")
  ...
  private def platformEnv(env: Map[String, String]): Config =
    ConfigFactory.parseMap(
      env.filter((key, _) => key.startsWith("NAKKA_") || key == "POD_IP").asJava
    )
