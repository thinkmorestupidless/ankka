...
object ClusterFormation:
  ...
  val FormationKey: String = "nakka.cluster.formation"
  val SeedNodesKey: String = "nakka.cluster.seed-nodes"
  ...
  def form(system: ActorSystem[?]): Unit =
    ...
    string(FormationKey, JoinSelfOrSeeds) match
      case JoinSelfOrSeeds =>
        ...
        else if boolean("nakka.join-self-if-no-seed-nodes", true) then
