...
private[nakka] final class DatabaseTimerScheduler(database: Database) extends TimerScheduler:
...
private[nakka] object TimerStore:
  ...
  def upsert(name: String, call: DeferredCall, dueAt: Instant): SqlFragment =
    SqlFragment.raw(
      "INSERT INTO nakka_timers (timer_name, component_id, method, payload, due_at, attempts) VALUES ("
    ) ++
  ...
  def delete(name: String): SqlFragment =
    SqlFragment.raw("DELETE FROM nakka_timers WHERE timer_name = ") ++ sql"$name"

  def byName(name: String): SqlFragment =
    SqlFragment.raw("SELECT timer_name FROM nakka_timers WHERE timer_name = ") ++ sql"$name"

  /** Due timers, oldest first, capped so one slow batch cannot starve the rest. */
  def due(now: Instant, limit: Int): SqlFragment =
    SqlFragment.raw(
      "SELECT timer_name, component_id, method, payload, attempts FROM nakka_timers WHERE due_at <= "
  ...
  def reschedule(name: String, attempts: Int): SqlFragment =
    ...
    SqlFragment.raw("UPDATE nakka_timers SET attempts = ") ++
