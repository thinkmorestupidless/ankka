...
class ClusterConfigSuite extends munit.FunSuite:
  ...
  test("the base and the overlay are both present at once") {
    ...
    assertEquals(config.getString("ankka.ask-timeout"), "10s")                      // base only
    assertEquals(config.getString("ankka.cluster.formation"), "join-self-or-seeds") // overlay only
  }

  test("an unknown mode fails at load, naming the modes that exist") {
    val e = intercept[IllegalArgumentException] {
      ClusterConfig.load(Map("ANKKA_CLUSTER_MODE" -> "nope"))
    }
    ...
    val e = intercept[ConfigException] {
      ClusterConfig.load(
        Map(
          ...
          "ANKKA_CLUSTER_POD_SELECTOR"   -> "a=b",
  ...
  test("kubernetes mode, fully supplied, resolves the overlay from the environment") {
    ...
    assertEquals(config.getString("ankka.cluster.formation"), "bootstrap")
    assertEquals(config.getBoolean("ankka.join-self-if-no-seed-nodes"), false)
  ...
  test("the base says nothing about how nodes find each other — the rule that makes it a base") {
    // Every jar ships a reference.conf; find ankka's by the header only it carries.
    val ours = getClass.getClassLoader
      .getResources("reference.conf")
      .asScala
      .map(url => Source.fromURL(url).mkString)
      .find(_.contains("Defaults for an ankka service"))
      .getOrElse(fail("could not find ankka's reference.conf on the classpath"))
  ...
  test("local mode with no environment is today's behaviour: random port, join self") {
    val config = ClusterConfig.load(none)
    assertEquals(config.getString("ankka.cluster.formation"), "join-self-or-seeds")
    assertEquals(config.getBoolean("ankka.join-self-if-no-seed-nodes"), true)
    assertEquals(config.getInt("pekko.remote.artery.canonical.port"), 0)
    assertEquals(config.getString("ankka.cluster.seed-nodes"), "")
  }
  ...
  test(
    ...
    val config = ClusterConfig.load(
      Map(
        "ANKKA_CLUSTER_PORT"       -> "17355",
        "ANKKA_CLUSTER_SEED_NODES" -> "pekko://a@h:1,pekko://b@h:2"
      )
    )
    assertEquals(config.getInt("pekko.remote.artery.canonical.port"), 17355)
  ...
  test(
    ...
    val raw = ConfigFactory
      .parseString("ankka.ask-timeout = 3s")
      .withFallback(ConfigFactory.load())
      .resolve()
    assert(!raw.hasPath("ankka.cluster.formation"), "premise: the raw config has no overlay")
    val layered = ClusterConfig.layered(raw, none)
    assertEquals(layered.getString("ankka.cluster.formation"), "join-self-or-seeds")
    assertEquals(layered.getInt("pekko.remote.artery.canonical.port"), 0)
    assertEquals(layered.getString("ankka.ask-timeout"), "3s", "the caller's own values survive")
  }
  ...
  test("layering a config the loader already produced changes nothing") {
    ...
    assertEquals(
      twice.getString("ankka.cluster.formation"),
      once.getString("ankka.cluster.formation")
    )
    assertEquals(
  ...
  test(
    ...
    val className =
      config.getString("pekko.management.health-checks.readiness-checks.ankka-extensions")
  ...
  test("the kubernetes overlay registers the version route where Pekko Management looks") {
    ...
    // silently ignored and /ankka/version is a 404 in every pod — which is how it was found.
    val config = ClusterConfig.load(
      env = Map(
        "ANKKA_CLUSTER_MODE"           -> "kubernetes",
        "POD_IP"                       -> "10.0.0.1",
    ...
    val className = config.getString("pekko.management.http.routes.ankka-version")
    val cls       = Class.forName(className)
    assert(
