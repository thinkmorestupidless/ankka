...
object LifecycleRules:
  ...
  def routeStatus(exposed: Boolean, baseDomain: Option[String], view: Option[RouteView]): String =
    if !exposed then ""
      ...
      "rejected: the operator has no base domain (ANKKA_BASE_DOMAIN); no route can be rendered"
    else
      view match
        case None => "pending"
        case Some(v) =>
  ...
  def unsupportedVersion(apiVersion: String, known: String): AnkkaServiceStatus =
    AnkkaServiceStatus(
      lifecycle = "Failed",
      detail =
