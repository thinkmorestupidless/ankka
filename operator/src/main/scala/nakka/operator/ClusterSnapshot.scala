 ...
 *
 * Note the two generations. `nakkaGeneration` is the annotation the operator wrote, carrying
 * nakka's own number. `k8sGeneration` is the API server's, and is only meaningful against
