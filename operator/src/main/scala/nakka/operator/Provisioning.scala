...
import nakka.operator.cnpg.DatabaseObservation
...
object Provisioning:
  ...
  def decide(spec: NakkaServiceSpec, observed: DatabaseObservation): ProvisioningPlan =
  ...
  private def objectRejected(state: nakka.operator.cnpg.CnpgObjectState): Option[String] =
