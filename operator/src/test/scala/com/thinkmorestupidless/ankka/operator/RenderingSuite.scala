...
import com.thinkmorestupidless.ankka.crd.{EnvEntry, AnkkaService, AnkkaServiceSpec}
...
class RenderingSuite extends munit.FunSuite:
  ...
  private def resource(
      ...
      uid: String = "uid-1",
      name: String = "cart",
      namespace: String = "ankka-checkout"
  ): AnkkaService =
  ...
  test("a descriptor label cannot override an identity label") {
    ...
    assertEquals(labels.get(Labels.ManagedByKey), Some("ankka"))
  ...
  test("a literal env var renders a value and a secret one renders a reference") {
    ...
    val secret = env.find(_.getName == "ANKKA_DB_PASSWORD").get
  ...
  private def containerOf(s: AnkkaServiceSpec) =
    deploymentFor(s).getSpec.getTemplate.getSpec.getContainers.get(0)

  test(
    "a port renders the http container port and ANKKA_HTTP_PORT — the same value"
  ) {
    ...
    assertEquals(
      c.getEnv.asScala.find(_.getName == "ANKKA_HTTP_PORT").map(_.getValue),
      Some("8080")
    )

  }

  test("no HTTP port renders no http container port and no ANKKA_HTTP_PORT") {
    val c = containerOf(spec.copy(port = None))
    assert(!c.getPorts.asScala.exists(_.getName == "http"), c.getPorts.toString)
    assert(!c.getEnv.asScala.exists(_.getName == "ANKKA_HTTP_PORT"))
  }
  ...
  test("the environment tells a node it is in Kubernetes, and how to find its peers") {
    ...
    assertEquals(value("ANKKA_CLUSTER_MODE"), Some("kubernetes"))
    assertEquals(value("ANKKA_CLUSTER_SERVICE"), Some("cart"))
    assertEquals(value("ANKKA_CLUSTER_CONTACT_POINTS"), Some("2"))
  ...
  test("required contact points are min(instances, 2): one must form alone, more must not") {
    for (count, expected) <- Vector(1 -> "1", 2 -> "2", 5 -> "2") do
      ...
      assertEquals(
        env.find(_.getName == "ANKKA_CLUSTER_CONTACT_POINTS").map(_.getValue),
        Some(expected),
        s"at $count"
      )
  }
  ...
  test("the platform's variables sit beside the descriptor's own env, not instead of it") {
    ...
    assert(env.contains("ANKKA_CLUSTER_MODE"), env.toString)
  }
  ...
  test("the injected port sits beside the descriptor's own env, not instead of it") {
    ...
    assert(names.contains("ANKKA_HTTP_PORT"), names.toString)
  }
  ...
  test("an exposed service renders one HTTPRoute, field for field") {
    ...
    assertEquals(route.getMetadata.getNamespace, "ankka-checkout")
    assertEquals(route.getMetadata.getOwnerReferences.get(0).getUid, "uid-1")
    assertEquals(route.getMetadata.getOwnerReferences.get(0).getKind, "AnkkaService")
  ...
  test(
    ...
    val removal = Action.RemoveHttpRoute("ankka-checkout", "cart", "uid-1")
