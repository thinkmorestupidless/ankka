...
class SchemaInitSuite extends munit.FunSuite:
  ...
  test("Rendering: the escape hatch renders no init container and no database envFrom") {
    ...
    val resource = {
      val r = new com.thinkmorestupidless.ankka.crd.AnkkaService
      r.setMetadata(
    ...
    val deployment =
      Rendering.deployment(resource, spec, "ankka-checkout", ProvisioningPlan.Supplied)
  ...
  test(
    ...
    assertEquals(podSpec.getInitContainers.get(0).getName, "ankka-schema")
    assertEquals(
