 ...
 *
 * `operator/src/main/resources/ankka/ddl` is a directory symlink to
 * `modules/runtime/src/main/resources/ankka/ddl`, the single canonical copy. This suite proves sbt
...
class SchemaResourceSuite extends munit.FunSuite:
  ...
  test("the journal DDL is non-empty and contains the expected table") {
    val content =
      scala.io.Source
        .fromInputStream(getClass.getResourceAsStream("/ankka/ddl/10-journal-postgres.sql"))
        .mkString
    assert(
