...
import nakka.crd.{NakkaService, NakkaServiceSpec}
...
class ServiceRenderingSuite extends munit.FunSuite:
  ...
  private def resource(spec: NakkaServiceSpec, uid: String): NakkaService =
    val r = new NakkaService
    r.setMetadata(
      new ObjectMetaBuilder().withNamespace("nakka-checkout").withName("cart").withUid(uid).build()
    )
    r.setSpec(spec)
    r
  ...
  private def actionsFor(s: NakkaServiceSpec, uid: String = "uid-1"): Vector[Action] =
  ...
  private def serviceFor(s: NakkaServiceSpec) =
    actionsFor(s)
      .collectFirst { case Action.EnsureService(svc) => svc }
      .getOrElse(fail("no Service"))

  private def deploymentFor(s: NakkaServiceSpec) =
    actionsFor(s)
  ...
  test("it is named after the service, in the project's namespace") {
    ...
    assertEquals(service.getMetadata.getNamespace, "nakka-checkout")
  }
  ...
  test("it carries the merged labels and the generation, as the Deployment does") {
    ...
    assertEquals(meta.getLabels.get(Labels.ManagedByKey), "nakka")
  ...
  test("no port means RemoveService — carrying the owner's uid — and never EnsureService") {
    ...
    assertEquals(
      ...
      Vector(Action.RemoveService("nakka-checkout", "cart", "uid-9"))
    )
  }
