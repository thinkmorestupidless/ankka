...
```bash
sbt 'operator/testOnly nakka.operator.ProvisioningSuite'
sbt 'operator/testOnly nakka.operator.CnpgRenderingSuite'
sbt 'operator/testOnly nakka.operator.SchemaInitSuite'
...
| `ServiceProjectionSuite` | `provisionDatabase` is `false` exactly when the descriptor declares any `NAKKA_DB_*` env var, and `true` otherwise. |
...
- The projected `NakkaService` carries `provisionDatabase`, and the CLI-visible status reports
...
own database, and `nakka services list` shows it `Ready`.
...
kubectl -n nakka-checkout get cluster,database,databaserole,secret,deploy
kubectl -n nakka-checkout describe nsvc cart | sed -n '/database/,+6p'
```
...
| `nakka services delete cart` | Deployment goes; **`Database` and its data remain** |
| re-apply `cart` | comes back with its old data, and the status says it recovered rather than provisioned |
| a descriptor carrying `NAKKA_DB_*` env vars | deploys, provisions nothing, no init container, status says `supplied` |

**Verifying the data actually persists** — the point of the whole feature:

```bash
kubectl -n nakka-checkout exec nakka-db-1 -c postgres -- psql -U postgres -c '\l' | grep cart
kubectl -n nakka-checkout exec nakka-db-1 -c postgres -- psql -U postgres -d cart -c '\dt'
# expect event_journal, snapshot, durable_state, projection_*, nakka_timers
```
...
- [ ] `cli`'s dependencies unchanged; `crd` still depends on no nakka module.
- [ ] `modules/runtime/src/main/resources/nakka/ddl/` is still the only real copy of the schema; the operator's path is a symlink to it.
