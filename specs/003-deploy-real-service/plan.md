# Implementation Plan: Deploy a Real nakka Service
...
resource, and used
by the operator to render the container port, the injected `NAKKA_HTTP_PORT`, the readiness probe and
the Service's target. They cannot disagree because there is one value. A descriptor that also sets
`NAKKA_HTTP_PORT` by hand is refused.

**A Service, and a probe that makes `Ready` mean something.** A `ClusterIP` Service named after the
service, selecting the *same* identity labels the Deployment selects, owned by the `NakkaService` so
...
| `-Dnakka.cluster.tests=off` was a no-op: tests fork, and the fork never saw the property | 001 | the "skipped" suites still passed — they just took seven minutes |
...
`SampleDeploymentClusterSuite` that runs a real nakka application in k3s; and a manual walkthrough.
...
**Constraints**: `sbt compile` stays warning-free; `sbt -Dnakka.cluster.tests=off test` must build
...
| **Schema and manifest move together** | PASS by construction, and called out explicitly: the `port` field lands in the Scala model and in `kustomization/components/crd/nakkaservice.yaml` in the same task. Feature 002 got this wrong and a real cluster caught it. |
...
└── NakkaService.scala             # + NakkaServiceSpec.port

kustomization/components/crd/
└── nakkaservice.yaml              # + port in the OpenAPI schema (same change, always)
...
└── Rendering.scala                # + the Service, containerPort, probe, NAKKA_HTTP_PORT,
                                   #   imagePullPolicy
...
controlplane-api/src/test/scala/nakka/controlplane/api/
