...
**`ankka-cluster-local.conf`** — peers are named, and default to oneself.

```hocon
ankka.cluster.formation = join-self-or-seeds
ankka.join-self-if-no-seed-nodes = on

pekko.remote.artery.canonical {
  hostname = "127.0.0.1"
  port = 0                          # random — what lets several services share a machine
  port = ${?ANKKA_CLUSTER_PORT}     # fix it on the node others will join
}
ankka.cluster.seed-nodes = ""                            # comma-separated; empty means join self
ankka.cluster.seed-nodes = ${?ANKKA_CLUSTER_SEED_NODES}
```

**Not `pekko.cluster.seed-nodes = ${?ANKKA_CLUSTER_SEED_NODES}`**, tempting as it is: an environment
variable is a string and that key is a list, so the substitution is a type error at load. The
formation step splits ankka's own comma-separated key and joins programmatically — the same call it
...
**`ankka-cluster-kubernetes.conf`** — peers are discovered through the API server.

```hocon
ankka.cluster.formation = bootstrap
ankka.join-self-if-no-seed-nodes = off     # here, joining self IS the split
...
`ANKKA_CLUSTER_MODE`, default `local`. The loader resolves the resource
`ankka-cluster-<mode>.conf`; if it does not exist, startup fails listing the modes that do.
...
`Ankka.start`'s `config` parameter changes its default from `ConfigFactory.load()` to the layered
loader. A caller passing its own `Config` — `AnkkaTestKit` does — bypasses it entirely, which is what
keeps every existing suite byte-for-byte as it is.

## Formation — one step, chosen by configuration

| `ankka.cluster.formation` | Does |
|---|---|
