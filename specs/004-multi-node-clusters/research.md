...
`kind-ankka` cluster in a scratch namespace, then removed. Standalone on purpose: feature 003 is
...
(R2), the formation step inside ankka itself (R3), the operator's new RBAC (R10), the Postgres
...
**Decision**: three layers inside `ankka-runtime`, selected by one environment variable:

| Layer | File | Holds |
|---|---|---|
| base | `reference.conf` (as today) | everything common — and **nothing** about how nodes find each other |
| local overlay | `ankka-cluster-local.conf` | join self by default; named seed nodes when given |
| Kubernetes overlay | `ankka-cluster-kubernetes.conf` | pod-IP binding, management, bootstrap via the Kubernetes API |

Selected by `ANKKA_CLUSTER_MODE` (`local` when unset), resolved as the resource
`ankka-cluster-<mode>.conf` — so a further means of execution is a new file and no code (FR-012).
...
ConfigFactory.defaultOverrides()
  ...
  .withFallback(ConfigFactory.parseResources(s"ankka-cluster-$mode.conf"))
...
## R2 — The local overlay must not cost what ankka has today
...
|---|---|---|
| `ANKKA_CLUSTER_PORT` | `0` (random) | fix it on the node others will join |
| `ANKKA_CLUSTER_SEED_NODES` | unset → join self | comma-separated addresses of existing nodes to join instead |
...
fixed port breaks something ankka relies on: several services, and several test suites, sharing one
...
The seed list is ankka's own comma-separated key, joined programmatically — not
...
`AnkkaTestKit` passes an explicit `Config` to `start`, bypassing the loader entirely, so every
existing suite runs exactly as before (SC-010). **Verified by reading** `AnkkaTestKit.scala:109-126`.
...
**Decision**: replace `joinSelfIfUnseeded` with one step driven by `ankka.cluster.formation`:
...
the bootstrap process!`). A warning on every local run trains people to ignore warnings, so ankka
decides from configuration which to start. Still one code path, still chosen by the overlay.

The Kubernetes overlay also sets `ankka.join-self-if-no-seed-nodes = off`. It must: in that mode a
self-join *is* the split this feature exists to prevent.

**Not verified**: this step inside ankka's own `Ankka.host` — the spike is standalone. First task of
the runtime story.
...
**The `nr = 1` result is not evidence that `1` is safe.** `kind-ankka` is one node with a fast,
...
on the HTTP port. ankka registers a second readiness check of its own — "the HTTP server is bound" —
...
## R9 — Losing a node: two different recoveries, and one setting ankka lacks
...
- The isolated node exiting — which is what lets Kubernetes restart it into a clean rejoin — depends
  on `pekko.coordinated-shutdown.exit-jvm = on`. The spike set it. **ankka's `reference.conf` does
...
**Decision**: per service, in its project's namespace, all owned by the `AnkkaService` so they go with
it (FR-030):
...
Not verified because it needs ankka's real schema and runtime, not a spike. It gets its own task and
...
They go in `ankka-runtime`, so the same build runs locally and deployed (FR-006, SC-006) — a local
