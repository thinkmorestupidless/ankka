# Specification Quality Checklist: An ankka Application Built Outside This Repository
...
cut from one tag (FR-007); **both `sbt new` and `ankka init`** over a single template — the user
chose both where the recommendation was `sbt new` alone, so `ankka init` is specified as a front
...
- The operator applies the schema from *its* image (`CnpgRendering` reads `/ankka/ddl` from its
