# Implementation Plan: An ankka Application Built Outside This Repository
...
`--directory=template`, or `file://` locally) and a new `ankka init` (which shells out to the same
...
The user's three answers shaped it: one version for everything; both `sbt new` and `ankka init`
...
by `-Dankka.template.tests` and `sbt` on `PATH`, ~4 minutes: publishLocal + an external sbt
...
|---|---|
| Single copy (DDL, manifests) | the template's `schema` task extracts from the jar — no second copy; `ankka init` runs the template, carries none |
...
│   └── template.md          # expansion, output, README commands, ankka init behaviour
└── tasks.md
```
...
controlplane/src/main/scala/ankka/controlplane/deploy/ServiceProjection.scala   # the runtime check → Left
cli/src/main/scala/ankka/cli/
├── Main.scala                                       # version; init
└── Init.scala                                       # NEW: the sbt new invocation
cli/src/test/scala/ankka/cli/TemplateSuite.scala     # NEW
ankka.g8/                                            # NEW: src/main/g8/{default.properties,…}
README.md, CLAUDE.md
```
...
4. **The template's two version sites are one parameter.** `ankka_version` writes both
   `build.sbt`'s `ankkaVersion` and `service.json`'s `runtime`.
5. **`ankka init` shells out; it does not embed.** If `sbt` is absent the message says so.
