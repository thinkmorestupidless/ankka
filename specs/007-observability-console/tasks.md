...
- [X] T021 [P] [US1] **Resolved: serve the component's own declared queries.** `GET /observability/query/{component}/{id}/{method}` in `ObservabilityEndpoint.scala` runs one of the handlers the component itself declared with `query`, and refuses a `command` with `405` and `"'add-item' is a command, not a query; the console only runs queries"`. The refusal is not a list of allowed names the console maintains: `HandlerBinding.readOnly` is set by which combinator declared the handler, and `query` accepts only a `ReadOnlyEffect`, so "a console cannot persist" is the compiler's guarantee carried onto the wire rather than a check that can drift. What this deliberately does *not* do is read arbitrary state: a component that declares no query shows none, because the alternative is a generic invoke-any-handler route or a direct read of the journal tables, and both are a larger grant than inspection needs. Verified live against the shopping cart: `queries ['total-quantity','get-cart']`, `get-cart` returned the real cart, `add-item` returned 405. ~~Original~~: `GET /observability/entities/{component}/{id}` in `ObservabilityEndpoint.scala`: current state, and `404` when the entity has never been created — never an empty state that looks real.
...
- [X] T037 [US1] Implement the **Invoke** panel (streaming completed later, with ConsoleStreamSuite) in `app.js`: route templates become a form, and the request goes to the **service's own HTTP address as an ordinary client** — which is what makes "cannot bypass an ACL" structural. A streaming response renders as it arrives, not on completion.
...
- [X] T041 [US1] Walked [quickstart.md](./quickstart.md) Tier 3 by hand with two services running, including `kill -9` on one (the stale entry was dropped from the listing and its file removed), and the ACL negative. The ACL half is also automated, in `controlplane/src/test/scala/.../ConsoleAclSuite.scala`, because it is the one claim in this feature worth a test that could falsify it: the console is not *checked* against an endpoint's `acl`, it is *subject* to it — the invoke panel makes an ordinary request to the service's own port carrying no privilege. The suite asks the same gated route twice, once through the console and once through a plain `HttpClient`, and asserts the answers agree **byte for byte** (403 without the key, 200 with it), plus that no plausible path under `/observability/` serves a route. 3/3.
...
- [X] T050 [US2] **Done — the automated half landed as `ControlPlaneClusterSuite` case 8 rather than a new suite**, because the cluster, the manifests and the deployed control plane it needs are already stood up there and a second k3s suite would have cost minutes to re-prove the same setup. It mints a 10-minute token for the `ankka-controlplane` ServiceAccount with `kubectl create token`, builds a fabric8 client that holds *only* that token, and asserts both directions against the API server: it can list pods and read a pod's log (the two calls `ankka services logs` actually makes, so this is the feature rather than a proxy for it), and it gets a **403 from the API server itself** deleting a pod and deleting a Deployment. 8/8 in 316s. Previously: `LogsRbacSuite` asserts the shipped manifest grants `get` on pods and pods/log and withholds every mutating verb, `pods/exec`, `pods/attach`, `pods/portforward` and all of `deployments`; the granted side was verified against kind with `kubectl auth can-i --as` the control plane's ServiceAccount (quickstart Tier 5, output recorded). **Remaining**: the automated form, so the granted side is covered in CI rather than by a command someone remembers — original: Write `controlplane/src/test/scala/com/thinkmorestupidless/ankka/controlplane/LogsClusterSuite.scala`: mint a token for the control plane's **own ServiceAccount** and read logs through it — not kind's admin credentials, which is the recorded reason an RBAC gap once reached a real deploy. Assert the **withheld** verbs too: that token must still be refused creating, deleting or exec'ing into a pod.
- [X] T051 [US2] Walked [quickstart.md](./quickstart.md) Tier 4 by hand against the kind cluster, including the no-credentials proof (SC-006): with `KUBECONFIG` unset and a throwaway `HOME`, `kubectl get pods -A` cannot reach a cluster at all, while `ankka services logs cart -p checkout --url … --token …` returned the deployed pod's real log lines, and the same call with a wrong token was refused `403 not permitted by this endpoint's acl`, exit 1. **The quickstart's own command was wrong and has been corrected**: `env -u KUBECONFIG HOME=$(mktemp -d)` does not isolate the CLI, because `Settings.path` resolves `~` through the JVM's `user.home`, which the launcher fixes at startup — the process went on reading the developer's real `~/.ankka/config.json`, and the first two attempts failed on a PKIX error that looked like a missing CA but was a config that was never consulted. `ANKKA_CONFIG` is the override, and the CA that has to be copied in is a TLS trust anchor rather than a credential — needed only because kind's gateway uses a private CA. See Tier 4 for the corrected form and both notes.
